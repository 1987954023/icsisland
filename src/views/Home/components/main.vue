     ...
     <div v-if="info.comicsviewtype===3" :class="`recommend-type-3`">
        <div class='item' v-for="(item,childIndex) in info.comicslist" :key="item.bigbook_id">
            ...
            <div class="text-group">
                <p class="item-name font-30">{{item.bigbook_name}}</p>
                <p class="item-hot font-24">人气：<span class="hot-hot">{{item.clickratelong | formatYi}}</span></p>
                <p class="item-text font-24">{{item.brief}}</p>
            </div>
        </div>
     </div>
</section>
